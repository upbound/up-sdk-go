...
const (
	// LicenseName is the required name for the singleton License resource.
	LicenseName = "uxp"

	// LicenseSecretName is the name of the secret that contains the license file.
	LicenseSecretName = "uxp-license"

	// LicenseSecretKeyDefault is the default key within the secret that contains the license file.
	LicenseSecretKeyDefault = "license.json"

	// PlanCommunity is the plan name for community edition licenses.
	PlanCommunity = "community"
)

// LicenseSecretRef references a Kubernetes secret containing the license file.
...
type LicenseSpec struct {
	...
	// +optional
	SecretRef *LicenseSecretRef `json:"secretRef,omitempty"`
}

// LicenseCapacity defines the capacity granted by the license.
...
// LicenseRestrictions defines environmental or operational constraints for the license.
type LicenseRestrictions struct {
	...
	// ClusterType specifies the type of cluster this license is valid for.
	// +optional
...
// LicenseUsage defines the cumulative usage data tracked by the system.
...
type LicenseStatus struct {
	...
	// InstalledComponents contains the status of the installed components.
	// This field is owned by the license-check-controller.
	// +optional
	InstalledComponents *InstalledComponentsStatus `json:"installedComponents,omitempty"`

	// Restrictions contains any usage restrictions associated with the license.
	// This field is owned by the license-check-controller.
	// +optional
	Restrictions *LicenseRestrictions `json:"restrictions,omitempty"`
...
// +kubebuilder:resource:scope=Cluster,shortName=lic,singular=license
...
// +kubebuilder:validation:XValidation:rule="self.metadata.name == 'uxp'",message="License object must be named 'uxp'"

// License is the singleton resource for UXP license configuration.
// There must be exactly one License resource, and it must be named 'uxp'.
type License struct {
